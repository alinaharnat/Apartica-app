...
const plansData = [
  {
    ...
    listingVisibility: 'Standard visibility',
  ...
  },
  {
    ...
    isHighlighted: true,
    frameStyle: {
      backgroundColor: 'bg-gradient-to-br from-purple-400 via-purple-600 to-indigo-400',
      borderRadius: 'rounded-[14px]',
      padding: 'p-7',
      margin: 'm-3',
    },
            ...
            {plansData.map((plan) => (
              ...
              >
                <div className={`rounded-t-[20px] overflow-hidden ${plan.colorTop}`}>
                  <div
                    className={`${plan.frameStyle.backgroundColor} ${plan.frameStyle.borderRadius} ${plan.frameStyle.padding} ${plan.frameStyle.margin}`}
                  >
                ...
                <div className={`p-6 ${plan.colorBottom} flex flex-col justify-between`}>
                  ...
                  <button
                    className={`w-full text-white py-2 rounded-md ${plan.buttonBg} transition ${
                      plan.title === 'Agency' && user?.userType?.includes('RentalAgency') ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                    disabled={plan.title === 'Agency' && user?.userType?.includes('RentalAgency')}
                    onClick={async () => {
                  ...
                  >
                    {plan.title === 'Agency' && user?.userType?.includes('RentalAgency') ? 'Subscribed' : plan.buttonText}
                  </button>
                </div>
              </div>
            ))}
