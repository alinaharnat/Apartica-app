    ...
    const handleGetStarted = async (e) => {
        ...
        if (storedUser) {
            try {
                ...
                const response = await axios.get(`/api/properties/user/${user.userId}`, config);
                console.log('Properties received:', response.data);

                const properties = response.data;
                // Count properties with isListed: true
                const listedPropertiesCount = properties.filter(
                  (property) => property.isListed === true
                ).length;

                // Check if user has 'RentalAgency' status
                const isRentalAgency = userData.userType?.includes('RentalAgency');

                // If not a RentalAgency and already has 2 listed properties, redirect to subscription plans
                if (!isRentalAgency && listedPropertiesCount >= 2) {
                ...
                navigate('/add-property'); // Fallback to add-property on error
            }
        } else {
